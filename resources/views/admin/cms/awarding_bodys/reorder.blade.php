...
@section('page_title', 'Reorder Awarding Bodys')
    ...
    <div class="card mb-5 mb-xl-5">
        <div class="card-header border-0">
            ...
            <div class="card-toolbar m-0">
                <a href="{{ route('awarding-body-list') }}" class="btn btn-danger"><i
                ...
                <ul class="list-group list-group-sortable">
                    @foreach ($list as $item)
                        <li id='{{ $item->id }}' class="list-group-item">
                            <i class="fas fa-bars reorder-icon"></i>{{ $item->title }}
                        </li>
                    @endforeach
                </ul>
            </div>
    ...
    <script type="text/javascript">
        $(function() {
            ...
            $('#btn-update-order').click(function() {
                $.ajax({
                    type: 'POST',
                    url: '{{ route('reorder-awarding-body') }}',
                    data: {
                        'items': $items,
                        '_token': '{{ csrf_token() }}',
                    },
