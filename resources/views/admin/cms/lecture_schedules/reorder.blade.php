...
@section('page_title', 'Reorder Lecture Schedules')
            ...
            <div class="card mb-5 mb-xl-5">
                <div class="card-header border-0">
                    ...
                    <div class="card-toolbar m-0">
                        <a href="{{ route('lecture-schedule-list') }}" class="btn btn-danger"><i class="fas fa-angle-left fs-4 me-2"></i> Go
                            Back</a>
                    </div>
                </div>

                @if (isset($data['lecture_list']) && count($data['lecture_list']) > 0)
                    <div class="card-body border-top p-9 custom_sortable">

                        <ul class="list-group list-group-sortable">
                            @foreach ($data['lecture_list'] as $item)
                                <li id='{{ $item->id }}' class="list-group-item">
                                    <i class="fas fa-bars reorder-icon"></i>{{ $item->lecture_date }} {{ $item->lecture_time}}
                                </li>
                            @endforeach
                        </ul>
                    </div>
    ...
    <script type="text/javascript">
        $(function() {
            ...
            $('#btn-update-order').click(function() {
                $.ajax({
                    type: 'POST',
                    url: '{{ route('reorder-lecture-schedule') }}',
                    data: {
                        'items': $items,
                        '_token': '{{ csrf_token() }}',
                    },
